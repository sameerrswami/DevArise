...
export async function PATCH(req, { params }) {
  try {
    ...
    const { id } = params;
    const updateData = {};
    if (typeof body.status === "string") updateData.role = body.status === "suspended" ? "suspended" : "STUDENT";
    if (typeof body.emailVerified !== "undefined") updateData.emailVerified = body.emailVerified ? new Date() : null;
    if (typeof body.role === "string" && ["STUDENT", "ADMIN", "RECRUITER"].includes(body.role)) updateData.role = body.role;
...
export async function GET(req, { params }) {
  try {
    ...
    const user = await prisma.user.findUnique({
      where: { id },
      select: {
        id: true, name: true, email: true, role: true, points: true, streak: true,
        createdAt: true, emailVerified: true, lastActive: true, contestRating: true,
