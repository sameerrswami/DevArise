...
export async function POST(request) {
  try {
    ...
    const { sessionId, aiFeature, conversationType, userMessage, aiResponse, context, importance = 1, topics = [], emotions = [], keyInsights = [] } = body;

    const user = await prisma.user.findUnique({ where: { email: session.user.email } });
    if (!user) return NextResponse.json({ error: 'User not found' }, { status: 404 });

    const memory = await prisma.conversationMemory.create({
      data: { userId: user.id, sessionId, aiFeature, conversationType, userMessage, aiResponse, context: context || {}, importance, topics, emotions, keyInsights },
    });
...
export async function GET(request) {
  try {
    ...
    const memories = await prisma.conversationMemory.findMany({
      where,
      orderBy: [{ importance: 'desc' }, { lastAccessed: 'desc' }, { createdAt: 'desc' }],
      take: limit,
    });
...
// PATCH /api/personalization/memory - Get contextual memory for AI responses
  ...
  try {
    ...
    const memories = await prisma.conversationMemory.findMany({
      where: {
        ...
        aiFeature,
        conversationType,
        OR: [{ topics: { hasSome: currentTopics } }, { importance: { gte: 3 } }],
      },
      orderBy: [{ importance: 'desc' }, { lastAccessed: 'desc' }, { createdAt: 'desc' }],
      take: limit,
    });
