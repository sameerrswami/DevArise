...
import GoogleProviderPkg from "next-auth/providers/google";
import bcrypt from "bcryptjs";
import prisma from "@/lib/prisma";

const CredentialsProvider = CredentialsProviderPkg.default ?? CredentialsProviderPkg;
const GoogleProvider = GoogleProviderPkg.default ?? GoogleProviderPkg;

export const authOptions = {
  trustHost: true,
  useSecureCookies: process.env.NODE_ENV === "production",
  
  // Dynamic base URL for Render/Vercel
  basePath: '',
  baseUrl: process.env.NEXTAUTH_URL || 
    `http${process.env.NODE_ENV === 'production' ? 's' : ''}://${process.env.VERCEL_URL || 'localhost:3000'}`,
  
  // Render.com compatible session handling
  session: {
    strategy: "jwt",
    maxAge: 30 * 24 * 60 * 60,
  },
