export class YouTubeService {
  ...
  async searchVideos(query, maxResults = 20) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') console.log("YouTube Search Response:", searchData);

      if (!searchData.items || searchData.items.length === 0) {
        if (process.env.NODE_ENV === 'development') console.error("No items found or API error:", searchData);
        return [];
      }
      ...
      if (process.env.NODE_ENV === 'development') console.log("YouTube Details Response:", detailsData);
